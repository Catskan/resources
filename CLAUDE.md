...
- **Secrets — KeePass-backed**: every `secrets.yml` and the `ansible_password` line in `connection.yml` use `lookup('viczem.keepass.keepass', '<group>/<entry>', '<password|username>')`. The DB path defaults to `/Volumes/Aurelien/Vault/Aurel-vault.kdbx` — the Synology NAS share `Aurelien` mounted via SMB (the share must be mounted before any run, otherwise `scripts/run.sh` aborts with "KeePass DB introuvable"). **Several stale copies of this `.kdbx` exist on the NAS** — including one at the former default path `/Volumes/home/Drive/Vault/`, which is no longer updated. Using a stale copy makes a run succeed with outdated secrets, which only surfaces when a certificate expires; see the comment in `scripts/run.sh`. Override via `KEEPASS_LOCATION` to point at a local copy. The master password is prompted at run-time by `scripts/run.sh` (no caching). Required collection: `viczem.keepass`. Python deps: `pykeepass`, `pyyaml` (the latter only for `scripts/inspect_keepass.py`).

> **`pykeepass` disparaît à chaque `brew upgrade ansible`.** Homebrew crée le venv d'Ansible avec `--without-pip --system-site-packages` et le recrée à neuf à chaque montée de version, emportant tout paquet installé à la main. Symptôme : n'importe quel play meurt sur `The lookup plugin 'viczem.keepass.keepass' failed to load: No module named 'pykeepass'`. Correctif : **`make deps`**, qui détecte le venv réellement utilisé (via `ansible --version`) et y réinstalle `pykeepass`. Ne pas installer dans le Python Homebrew système : il est `EXTERNALLY-MANAGED`.
